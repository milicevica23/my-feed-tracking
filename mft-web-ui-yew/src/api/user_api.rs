...
pub async fn api_register_user(username: String, pwd_clear: String) -> Result<(), String> {
    ...
    info!("{:#?}", serde_json::to_string(&user).unwrap());
...
pub async fn api_login_user(username: String, pwd_clear: String) -> Result<User, String> {
    let login_payload = LoginPayload {
        username: username,
        pwd: pwd_clear,
    };
    let response = match http::Request::post(&format!("{}{}", BACKEND_URL, "api/login"))
        .header("Content-Type", "application/json")
