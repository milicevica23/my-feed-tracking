            ...
            spawn_local(async move {
                match form.validate() {
                    Ok(_) => {
                        ...
                        let res = api_login_user(email_input.to_string().into(), password_input.to_string().into()).await;
                        match res {
                            Ok(_) => {
    ...
    <section class="bg-ct-blue-600 min-h-screen grid place-items-center">
      <div class="w-full">
        ...
        <h2 class="text-lg text-center mb-4 text-ct-dark-200">
          ...
          >
            <FormInput label="Email" name="email" input_type="email" input_ref={email_input_ref} handle_onchange={handle_email_input} />
